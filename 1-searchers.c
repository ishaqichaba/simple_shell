...
int check_path_argv(char *argv0, char **path, int *track, int *flag)
{
	...
	*flag = 0;
	if(!(search_cmd(argv_filter(argv0),path, track)))
	{
		...
		_strcat_ptr(&temp, "/");
		if((*flag = _strcmp(argv0, argv_filter(argv0))) == 0 || _strcmp(argv0, _strcat_ptr(&temp, argv_filter(argv0))) == 0)
		{
			free(temp);
			return 0;
		}
	...
	if(!search_in_other_dir(argv0, flag)) return 0;
	return 1;
}
...
int search_in_other_dir(char *argv0, int *flag)
{
	...
	*flag = 1;
	...
	{
		path = getcwd(buff, 256); goto E;
	}
	path = _strdup_(argv0);
	path[_strlen(path) - _strlen(argv_filter(argv0)) - 1] = '\0';
	if(!_strcmp(path, "."))
	{
		free(path); path = getcwd(buff, 256);
	}
E:
	if((direct = opendir(path)) == NULL)
	{
		closedir(direct); free(path); return 1;
	}
	while((file = readdir(direct)))
	{
		if (_strcmp(argv_filter(argv0), file->d_name) == 0)
		{
			closedir(direct); free(path); return 0;
		}
	}
	closedir(direct); free(path); return 1;
}